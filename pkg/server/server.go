...
func Start(ctx context.Context, opts Options) (int, error) {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		if err := createDefaultConfig(configPath, opts.DefaultConfig); err != nil {
	...
	if err := initAdminUser(cfg); err != nil {
		return 0, fmt.Errorf("failed to initialize admin user: %w", err)
	}
	...
	if err := initDefaultScenarios(opts.DefaultScenarios, resetScenarios); err != nil {
...
func createDefaultConfig(path string, data []byte) error {
...
func initDefaultScenarios(data []byte, reset bool) error {
	var count int64
